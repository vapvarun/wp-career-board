...
2. As `varundubey` (admin), seed credits for `employer.figma` to exactly 2: insert into `wp_wcb_credit_ledger` with `user_id=<figma-id>`, `entry_type=grant`, `amount=2`, `note='Smoke journey credit-decrement seed'`
...
6. Verify a debit row landed in the ledger: `wp db query "SELECT entry_type, amount FROM wp_wcb_credit_ledger WHERE user_id=<figma-id> ORDER BY id DESC LIMIT 1"` → expect `entry_type=debit, amount=1` (or matching package definition)
