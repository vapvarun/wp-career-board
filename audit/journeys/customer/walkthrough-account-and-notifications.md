...
1. As `sarah.chen`, navigate to `$WCB_SITE/candidate-dashboard/?autologin=sarah.chen` → expect HTTP 200 and the dashboard shell `div.wcb-dashboard[data-wp-interactive="wcb-candidate-dashboard"]` (NOT the `.wcb-db-gate` "Please sign in" fallback). _(blocks/candidate-dashboard/render.php:18-28, 268-401)_
...
5. Reload `$WCB_SITE/candidate-dashboard/?autologin=sarah.chen`, re-open `#wcb-tab-settings` → expect `#wcb-account-name` to still show the saved display name (a fresh `GET {apiBase}/account` / `render.php` seed confirms persistence to the WP user record). _(class-account-endpoint.php:87-96; render.php:197-198)_
...
8. As `employer.figma`, ensure they own a published job so the employer email has a recipient (the new-application email goes to the job's `post_author`): navigate to `$WCB_SITE/employer-dashboard/?autologin=employer.figma` and post one via the Post a Job flow (`POST {apiBase}/jobs`, CREATABLE), or reuse an existing `employer.figma`-owned `wcb_job`. Note its job ID `{jobId}`. _(api/endpoints/class-jobs-endpoint.php:48 CREATABLE; recipient logic class-email-app-received.php:79-83)_
...
- **`apiBase`** is `untrailingslashit( rest_url('wcb/v1') )` → `$WCB_SITE/wp-json/wcb/v1`; every dashboard fetch sends `X-WP-Nonce` from `state.nonce` (`wp_create_nonce('wp_rest')`) — `render.php:164-165`. All `view.js` fetches route through `@wcb/fetch` (15s AbortController).
