...
(configurable in the **Career Board → Settings → Currency** tab).
Multi-currency boards still display each job in its own currency,
but the filter applies the comparison after a normalized conversion.
