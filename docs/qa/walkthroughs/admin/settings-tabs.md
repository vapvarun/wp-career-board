...
1. As `varundubey`, snapshot the full option before any change: `BEFORE=$(wp option get wcb_settings --format=json --path=$WCB_PATH)` → note `notification_email` (a Notifications key) and `jobs_archive_page` (a Pages key) for the merge assertions.
...
11. **Merge assertion #3 (Antispam save must not clobber earlier tabs).** Re-read `wcb_settings` → expect `jobs_per_page: 12`, `notification_email: admin@example.test`, and `jobs_archive_page` still equal to `$BEFORE` — the antispam admin-post save left every non-captcha key intact.
