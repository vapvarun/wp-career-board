...
1. As `varundubey`, navigate to `$WCB_SITE/wp-admin/admin.php?page=wcb-emails&autologin=varundubey` → expect an immediate redirect to `$WCB_SITE/wp-admin/admin.php?page=wcb-settings&tab=emails` (the `wcb-emails` submenu is a redirect stub, `admin/class-admin.php:649-656`), HTTP 200, Emails section active.
...
10. **Merge tags resolve in a test send.** Click the row's "Send test" `button.wcb-email-test-btn[data-email-id="application-received"]` → expect `POST $WCB_SITE/wp-json/wcb/v1/admin/emails/test` body `{email_id:"application-received"}` returning HTTP 200 `{ sent: <bool>, to: "<admin email>", logged: <int> }` (`AdminEndpoint::test_send_email()`, gated on ability `wcb/manage-settings`, `api/endpoints/class-admin-endpoint.php:57-233`). Assert `logged` is `>= 1`.
