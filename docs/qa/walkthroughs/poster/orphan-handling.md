...
2. Save a company profile: `POST $WCB_SITE/wp-json/wcb/v1/employers` `{"name":"Adopt Test Co"}` → expect HTTP 200; `_wcb_company_id` user meta is now set (capture `<company-id>`). Backfill lives in `EmployersEndpoint::create_item()` → `backfill_orphan_jobs()` (idempotent, bounded to the author's own jobs).
...
4. `GET $WCB_SITE/wp-json/wcb/v1/employers/me/jobs` → the previously-orphaned job now appears in the list (before the fix it was invisible because My Jobs queries by company id).
...
6. As `wcbp_p5_candidate`, apply to any published job → confirm the new application appears in `$WCB_SITE/candidate-dashboard/` → My Applications, status "Submitted". Capture `<app-id>`.
...
9. As `wcbp_p5_candidate`, reload `$WCB_SITE/candidate-dashboard/` → click My Applications. The row for the deleted job must:
