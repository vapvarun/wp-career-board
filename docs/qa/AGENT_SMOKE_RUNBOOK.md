...
- Working directory: `/Users/varundubey/Local Sites/job-portal/app/public`
- Site URL: `http://job-portal.local`
- WP-CLI: `wp --path="$WP_PATH" <cmd>` where `WP_PATH=/Users/varundubey/Local Sites/job-portal/app/public`
...
> D rows are sourced from the last 30 git commits (2026-05-09 audit). Every customer-visible fix from now on adds a D row in the same PR. After 2 clean releases, a D row graduates into C/E.
...
> **Pro implementation status snapshot (v1.1.0, 2026-05-09 audit):** resume is `complete` (REST + admin + UI all wired, 18 hooks); fields, ai, maps, boards, credits, notifications-bell, notifications-pro are `partial` (REST + some admin, limited UI); alerts, analytics, feed, migration, pipeline, pwa are `stub`. Walk the complete + 7 partial modules live; stub modules are `skipped` with the note above.
