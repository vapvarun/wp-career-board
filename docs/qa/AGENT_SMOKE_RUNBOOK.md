...
| D.orphan-company-link-on-post | A job posted from the standalone Post-a-Job page was saved with NO `_wcb_company_id` whenever the employer's reciprocal `_wcb_company_id` **user** meta was unset (companies created by import / admin / migration only ever have the post-side link). The job looked fine until the employer opened their dashboard, whose render self-heals the user meta — `/employers/me/jobs` then switched from the author-scoped branch to the company-scoped `meta_query` and the job vanished permanently. Fixed in 1.7.1: `JobsEndpoint::create_item()` resolves through `CompanyMetaShape::resolve_company_id()`; the admin metabox save no longer strips the link; `backfill_orphan_jobs()` also runs on company update; DB 1.3.0 sweeps existing orphans. Basecamp 10134657106. | `wp user meta delete <employer-id> _wcb_company_id` (leave their `wcb_company` published). As that employer, `POST /wcb/v1/jobs` from the STANDALONE post page. Assert `wp post meta get <job-id> _wcb_company_id` equals the owned company id and `_wcb_company_name` its title. Then **load the employer dashboard FIRST**, and only then `GET /wcb/v1/employers/me/jobs` — the job must still be listed (this ordering is what fails on the broken build). As anonymous, `GET /wcb/v1/jobs/<job-id>` must carry non-empty `company_tagline` / `company_industry` / `company_size_label` / `company_hq`. Finally, as admin press **Update** on that job in `wp-admin` without touching the Company select — `_wcb_company_id` must survive. Zero new fatals in debug.log. |
| D.application-custom-answers | Answers to questions registered via the `wcb_application_form_fields_groups` filter were persisted as `_wcb_application_field_<key>` postmeta but read back by nothing — no REST envelope, no employer dashboard section, no admin metabox — so employers never saw what applicants answered. The endpoint also read `$_POST['custom_fields']` directly, silently dropping answers from JSON (non-multipart) REST clients. Fixed in 1.7.1: persistence goes through the shared `FormCustomFields::save_values()` (with a key prefix), and the new `FormCustomFields::labelled_values()` feeds the applications REST envelope, the employer dashboard detail pane, and a new `application/custom-answers` admin widget. Basecamp 10134659689. | Register one question through `wcb_application_form_fields_groups` in an mu-plugin fixture. Apply to a job with an answer. Assert (1) `wp post meta get <app-id> _wcb_application_field_<key>` holds the answer, (2) `GET /wcb/v1/applications/<app-id>` as employer AND as admin returns a `custom_fields` array containing `{key,label,type,value}` for it, (3) `GET /wcb/v1/employers/me/applications` carries the same `custom_fields` on the row, (4) the wp-admin application edit screen shows an "Application answers" section with the label and value, (5) applying with a JSON (non-multipart) body persists the answer identically. Zero new fatals in debug.log. |
| D.kanban-default-stage | A submitted application never appeared on the employer's Kanban. Five defects stacked: (D1) nothing wrote `_wcb_stage_id` on `wcb_application_submitted`; (D2) NOTHING ever created stages, so a board existed with zero Kanban columns and a write-only fix would have had nothing to write; (D3) `GET /jobs/{id}/kanban` resolved the board from raw `_wcb_board_id` meta instead of the `wcb_job_board_id` filter, returning zero columns for every job created without an explicit board; (D4) deleting a stage orphaned its applications behind an id with no row; (D5) "first stage" was undefined because `ORDER BY sort_order` had no direction and no tiebreak while `create_stage` defaults `sort_order` to 0. Fixed in Pro 1.7.1: new `StageRepository` is the single stage model, boards seed 5 default stages (Screening / Interview / Offer / Hired / Rejected), `PipelineModule::assign_initial_stage()` stamps the first stage at priority 5, column 1 also lists stage-less rows, `delete_stage()` re-homes instead of orphaning, and DB 1.7 migrates existing rows. Basecamp 10134657558. | With Free + Pro active: `wp option get wcbp_db_version` must be `1.7` or higher. Pick a published job with NO `_wcb_board_id` meta; `wp eval "echo \WCB\Pro\Modules\Pipeline\StageRepository::board_for_job( <job-id> );"` must return a board id > 0 (default-board fallback), and that board must have exactly 5 stage rows nobody created by hand. Apply as a candidate via `POST /wcb/v1/jobs/<job-id>/apply`; assert `wp post meta get <app-id> _wcb_stage_id` equals the board's LOWEST-`sort_order` stage id (ties broken by lowest `id`). As the employer, `GET /wcb/v1/jobs/<job-id>/kanban` must return 5 columns with `<app-id>` in `columns[0].applications`, `columns[0].total >= 1`, and absent from every other column. Then `wp post meta delete <app-id> _wcb_stage_id` and re-GET — the card must STILL be in column 1 (unassigned fallback, filter `wcbp_kanban_include_unassigned`). Zero new fatals in debug.log. |
| D.admin-job-geocodes | Geocoding hung off the `wcb_job_created` action, which fires from exactly one place - Free's REST create route (`api/endpoints/class-jobs-endpoint.php`). A job an admin typed into wp-admin never received `_wcb_lat`/`_wcb_lng` and never appeared on the job map. Firing `wcb_job_created` from the admin save path was rejected as the fix: it also drives the job-pending email, BuddyPress activity, Pro AI embeddings and Pro job alerts, so every admin edit would have spammed applicants. Fixed in Pro 1.7.1: geocoding listens on `save_post_wcb_job` / `save_post_wcb_resume` and dispatches the async `wcbp_geocode_post` worker; `_wcb_geocoded_for` fingerprints the location string so an unrelated save costs nothing and a location edit re-geocodes. Basecamp 10134662318. | Snapshot `SELECT COUNT(*) FROM wp_wcb_notifications`, the mail count, and (if BuddyPress is active) `SELECT COUNT(*) FROM wp_bp_activity`. Create a job WITHOUT the REST route: `wp post create --post_type=wcb_job --post_status=publish --porcelain`, `wp post term set <job-id> wcb_location "Berlin"`, then `wp post update <job-id> --post_excerpt=x`. Assert a pending `wcbp_geocode_post` event carrying the job id exists AND `_wcb_lat` is still empty (the save must NOT make an inline external HTTP call). Run `wp cron event run wcbp_geocode_post`; assert `_wcb_lat` / `_wcb_lng` are numeric and `_wcb_geocoded_for` equals `Berlin`. Re-read all three counters from the snapshot - ALL must be unchanged and zero mail sent (any increase means `wcb_job_created` was fired from the save path). Finally `wp post update <job-id> --post_excerpt=y` must queue NO new geocode, while changing the location term must queue one. |
| D.resume-map-pins | The resume map was permanently empty on every site: no code path ever wrote `_wcb_lat` on a `wcb_resume`, so resume geocoding simply did not exist. The block compounded it by reading the pre-1.1.0 importer meta keys (`_wcb_candidate_title` / `_wcb_location`) instead of the keys native resumes write, so even a hand-geocoded resume rendered a nameless, location-less pin; and its empty state told owners the resumes "can be backfilled from the Resumes admin" - a screen that does not exist. Fixed in Pro 1.7.1: `save_post_wcb_resume` geocodes through the shared worker on the GLOBAL provider (resumes are not board-scoped), the block reads `_wcb_resume_headline` / `_wcb_resume_location` with the importer keys as fallbacks, the toolbar and status live region are suppressed in the empty state exactly as job-map does, and the icons come from `Icon::svg()` instead of a raw `&#x2316;` glyph. Basecamp 10134662279. | As a candidate, save a resume with a location. Assert a pending `wcbp_geocode_post` event carrying the resume id, then `wp cron event run wcbp_geocode_post` and assert numeric `_wcb_lat` / `_wcb_lng` plus `_wcb_geocoded_for` equal to the saved location. Render the page hosting the `wcb/resume-map` block: the pins payload must contain the resume with a NON-EMPTY `title` (from `_wcb_resume_headline`) and `location` (from `_wcb_resume_location`), `.wcb-rm-empty` must be absent, and `.wcb-jm-toolbar` present. Then hide every resume's `_wcb_lat` and re-render: `.wcb-rm-empty` must render while `.wcb-jm-toolbar`, `.wcb-rm-status` and `.wcb-job-map-canvas` are ALL absent, and the empty-state body must not mention a "Resumes admin" backfill. Assert the raw string `&#x2316;` appears nowhere in the output. |
| D.employer-active-job-limit | New in 1.7.1: sites can cap concurrent live jobs per employer via `wcb_employer_active_job_limit` (0 = unlimited, the default, so the feature is inert until filtered). Two contracts break silently: the cap MUST be skipped wholesale when `wcb_credits_enabled` is true (paid posting already meters volume - charging a credit then refusing the post is the worst of both models), and the republish check MUST exclude the job being republished from its own count (otherwise an employer at the cap can never reopen anything). Checked in `POST /wcb/v1/jobs` and in `PUT /wcb/v1/jobs/{id}` when a listing flips back to publish. Basecamp 10134733032. | Add an mu-plugin with `add_filter( 'wcb_employer_active_job_limit', fn() => 5 );` plus `add_filter( 'wcb_credits_enabled', '__return_false', 99 );`. With the employer over 5 published jobs, `POST /wcb/v1/jobs` must return 403 `wcb_active_job_limit` with `data.limit`=5, a real `data.count`, and a message naming both numbers. Close listings under the cap and re-POST -> 201. Back at the cap, `PUT /wcb/v1/jobs/<closed-job-id>` with `status=publish` -> 403; close one OTHER listing and repeat -> 200 (self must not be double-counted). Drop the credits filter so credits are enabled and POST while still over the cap -> 201. Remove the fixture entirely -> 201 (default 0 = unlimited). Zero new fatals in debug.log. |
